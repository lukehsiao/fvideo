...
impl FvideoDummyTwoStreamServer {
    ...
    fn crop_x264_pic(&mut self, gaze: &mut GazeSample, width: u32, height: u32) {
        ...
        for i in 0..3 {
            let mut offset: isize = match self.triggered {
                false => (self.pic_black.pic.img.i_stride[i] * top / csp_height[i])
                    .try_into()
                    .unwrap(),
                true => (self.pic_white.pic.img.i_stride[i] * top / csp_height[i])
                    .try_into()
                    .unwrap(),
            };
            ...
            unsafe {
                offset_plane[i] = match self.triggered {
                    false => self.pic_black.pic.img.plane[i].offset(offset),
                    true => self.pic_white.pic.img.plane[i].offset(offset),
                };

                // Used to prevent reading off the right or bottom edges
                let base_ptr: *const u8 = match self.triggered {
                    false => &*self.pic_black.pic.img.plane[i],
                    true => &*self.pic_white.pic.img.plane[i],
                };

                // These calculations doesn't matter which pic, they are the same size.
                let max_offset = base_ptr as usize + self.pic_black.plane_size[i];

                let size: usize = {
                    let right = (left + width as i32) / csp_width[i];

                    if right > self.pic_black.pic.img.i_stride[i] {
                        (self.fg_pic.pic.img.i_stride[i]
                            - (right - self.pic_black.pic.img.i_stride[i]))
                            .try_into()
                            .unwrap()
                    } else {
